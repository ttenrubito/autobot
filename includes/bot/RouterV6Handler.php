<?php
/**
 * RouterV6Handler - Multi-tenant message router (extends V5 architecture)
 * 
 * Changes from V5:
 * - StoreConfigService: Feature toggles per channel
 * - Configurable business rules (trade-in rates, etc.)
 * - Dynamic product category keywords
 * 
 * Core services (inherited from V5):
...
class RouterV6Handler implements BotHandlerInterface
{
    protected $db;
    ...
    public function __construct()
    {
        ...
        // Initialize Function Executor for LLM Function Calling
    ...
    public function handleMessage(array $context): array
    {
        ...
        try {
            ...
            // ==================== 🧠 LLM-FIRST APPROACH (V5 CORE!) ====================
            // ทุก message จะถูกส่งให้ LLM วิเคราะห์ก่อน พร้อม function calling
            // LLM จะจัดการทุกอย่าง: product search, follow-up, multi-intent, chitchat
            ...
            if ($this->isLlmEnabled($config)) {
                ...
                $llmResult = $this->handleWithFunctionCalling($context, $config);

                if ($llmResult !== null) {
                    // LLM handled successfully
                    if (!empty($llmResult['reply'])) {
                        $replyText = is_array($llmResult['reply'])
    ...
    protected function handleProductSelectionByName(string $productNameQuery, array $config, array $context): array
    {
        ...
        if (!$caseId) {
            // No active case - try product search instead (show Product Card, not checkout)
        ...
        if (empty($history)) {
            // No products in history - try product search (show Product Card)
            return $this->handleProductSearch(['keyword' => $productNameQuery], $config, $context);
        }
        ...
        // ✅ FIX: Found in history - show Product Card first, not checkout directly
        // Customer needs to confirm via Product Card button before checkout
        $product = [
            'ref_id' => $matchedProduct['product_ref_id'],
            'id' => $matchedProduct['product_ref_id'],
        ...
        // ✅ FIX: Show Product Card instead of startCheckout directly
        // Track view first
    ...
    protected function handleFallback(array $config, array $context, array $templates): array
    {
        ...
        // ==================== 🚀 LLM + FUNCTION CALLING (V5 NEW!) ====================

        // Try LLM with Function Calling for smart response
        if ($this->isLlmEnabled($config)) {
            $functionResult = $this->handleWithFunctionCalling($context, $config);
            if ($functionResult !== null) {
                return $functionResult;
            }

            // Fallback to regular LLM if function calling fails
            $llmResponse = $this->handleWithLLM($context, $config);
            if ($llmResponse) {
                return ['reply' => $llmResponse];
            }
    ...
    protected function shouldProcessMessage(string $text, string $platformUserId, int $channelId, string $traceId, array $config = []): array
    {
        ...
        // 5.5 🚫 Ultra-Short Message Filter (Dynamic Whitelist Approach)
        // ข้อความสั้นมาก (<= 6 chars) ต้องมี business keyword ถึงจะผ่าน
        $len = mb_strlen($text, 'UTF-8');
        if ($len <= 6) {
            // === Dynamic Keywords from Config ===
            $customActionKw = $gatekeeperCfg['action_keywords'] ?? [];
            $customProductKw = $gatekeeperCfg['product_keywords'] ?? [];
            $customBrandKw = $gatekeeperCfg['brand_keywords'] ?? [];

            // Base action keywords (universal)
            $actionKeywords = 'ซื้อ|ดู|ขอ|หา|เอา|จอง|สนใจ|ถาม|ส่ง|โอน|จ่าย|ผ่อน|ราคา|สวัสดี|hello|hi|help|price|buy|order';

            // Base product keywords (can be overridden by config)
            $productKeywords = 'นาฬิกา|แหวน|สร้อย|ทอง|เพชร|กำไล|จี้|ต่างหู|กระเป๋า|watch|ring|gold|diamond';

            // Base brand keywords (can be overridden by config)  
            $brandKeywords = 'rolex|omega|patek|cartier|hublot|audemars|chanel|hermes|lv|gucci';

            // Merge custom keywords
            if (!empty($customActionKw)) {
                $actionKeywords .= '|' . implode('|', array_map('preg_quote', $customActionKw));
            }
            if (!empty($customProductKw)) {
                $productKeywords .= '|' . implode('|', array_map('preg_quote', $customProductKw));
            }
            if (!empty($customBrandKw)) {
                $brandKeywords .= '|' . implode('|', array_map('preg_quote', $customBrandKw));
            }

            // Build final pattern: action OR product OR brand OR number(3+digits)
            $intentPattern = "/({$actionKeywords}|{$productKeywords}|{$brandKeywords}|\\d{3,})/iu";
            $hasBusinessIntent = preg_match($intentPattern, $text);

            if (!$hasBusinessIntent) {
                Logger::debug('[GATEKEEPER] Short message without business intent, skipping', [
                    'trace_id' => $traceId,
                    'text' => $text,
                    'length' => $len,
                ]);
                return ['should_process' => false, 'reason' => 'short_no_intent'];
            }
        }
        ...
        // 6.5 🚫 Absolute Low Score Filter - ถ้า score ต่ำมากๆ skip ทันทีไม่ต้องรอ timing
        // เช่น "ยังไงบ้าง", "พี่ขาาาา", "แล้วไงล่ะ" → infoScore ~0.05
        $absoluteMinScore = (float) ($gatekeeperCfg['absolute_min_score'] ?? 0.15);
        if ($infoScore < $absoluteMinScore) {
            Logger::debug('[GATEKEEPER] Absolute low score, skipping', [
                'trace_id' => $traceId,
                'text' => $text,
                'info_score' => $infoScore,
                'min_score' => $absoluteMinScore,
            ]);
            return ['should_process' => false, 'reason' => 'absolute_low_score'];
        }
    ...
    protected function calculateInfoScore(string $text, array $config = []): float
    {
        ...
        // === Early Exit: Pure filler particles (ครับ ค่ะ นะ จ้า) ===
        if (preg_match('/^(ครับ|ค่ะ|คะ|คับ|นะคะ|นะครับ|จ้า|จ้ะ|จ๊า|ค่า|เค|โอเค|ok|okay|k|kk)+[!?.\s]*$/iu', $text)) {
            return 0.0;
        }
    ...
    protected function looksLikeProductQuery(string $text): bool
    {
        ...
        // ✅ Added กำไร (common typo for กำไล)
        if (preg_match('/(นาฬิกา|แหวน|สร้อย|กำไล|กำไร|จี้|ต่างหู|เพชร|ทอง|ทองคำ|เงิน|พลอย|ไข่มุก)/u', $text)) {
            return true;
        }
                            ...
                            if ($matchedPawn) {
                                ...
                                if (count($activeInstallments) > 0 && $slipAmountFloat > 0) {
                                    ...
                                    if ($matchedInstallment) {
                                        ...
                                        $summary = [];
                                        if (count($activePawns) > 0)
                                            $summary[] = "จำนำ " . count($activePawns) . " รายการ";
                                        if (count($activeInstallments) > 0)
                                            $summary[] = "ผ่อนชำระ " . count($activeInstallments) . " รายการ";
    ...
    /**
     * Handle message with LLM + Function Calling
     * 
     * This is the key V5 feature: Instead of just generating text,
     * the LLM decides which function to call and with what arguments.
     * 
     * @return array|null Response array or null if should fallback
     */
    protected function handleWithFunctionCalling(array $context, array $config): ?array
    {
        $text = $context['message']['text'] ?? '';
        ...
        // Use Gemini 2.5 Flash for function calling (good balance of speed and capability)
        ...
        $functionDeclarations = FunctionRegistry::getDeclarations();

        // Build system prompt for function calling
        $systemPrompt = $this->buildFunctionCallingSystemPrompt($config);
        ...
        // Build Gemini request with function declarations
        $payload = [
            'contents' => [
                [
                    'role' => 'user',
            ...
            'generationConfig' => [
                'temperature' => 0.3, // Lower temperature for more deterministic function selection
                'maxOutputTokens' => 1024,
            ],
            'toolConfig' => [
                'functionCallingConfig' => [
                    'mode' => 'AUTO' // Let model decide: function call or text response
                ]
            ]
        ];
        ...
        if ($functionCall) {
            ...
            $this->functionExecutor->setContext($config, $context);
            $result = $this->functionExecutor->execute($functionName, $arguments);

            // Handle the result based on type
            return $this->processFunctionResult($result, $functionName, $config, $context);
        }
        ...
        if ($textResponse) {
            ...
            return ['reply' => trim($textResponse)];
        }
    ...
    /**
     * 🚀 Check if message is a follow-up question that needs context
     * 
     * These are short messages that reference previous conversation context
     * like "that one", "other color", "this product", etc.
     */
    protected function isFollowUpQuestion(string $text): bool
    {
        $text = mb_strtolower(trim($text), 'UTF-8');
        $len = mb_strlen($text, 'UTF-8');

        // Too long = probably not a follow-up
        if ($len > 50) {
            return false;
        }
        ...
        $followUpPatterns = [
            ...
            '/อัน(นี้|นั้น|ไหน|โน้น)/u',
            '/ตัว(นี้|นั้น|ไหน)/u',
            '/ชิ้น(นี้|นั้น|ไหน)/u',
            '/(เมื่อกี้|ก่อนหน้า|ที่เพิ่ง)/u',

            // Asking about variants
            '/(สี|ขนาด|ไซส์)(อื่น|อื่นๆ)/u',
            '/มี(สี|ขนาด|ไซส์|แบบ)(อื่น|ไหน|อะไร)/u',

            // Price/installment follow-up
            '/(อันนี้|ตัวนี้|ชิ้นนี้)(ราคา|ผ่อน|ลด)/u',
            '/ผ่อน(ได้|ไหม|ยังไง)$/u',
            '/ลด(ได้|ไหม|หน่อย)$/u',

            // Short stock questions
            '/^(มี|เหลือ)(ไหม|มั้ย|อยู่ไหม)$/u',
            '/^ยัง(มี|เหลือ)(ไหม|มั้ย|อยู่)$/u',

            // Confirmation/selection
            '/^เอา(อัน|ตัว|ชิ้น)(นี้|นั้น|ที่\d)$/u',
            '/^(อัน|ตัว|ชิ้น)(ที่\s?\d+|แรก|สุดท้าย)/u',

            // Comparison
            '/อัน(ไหน|นี้|นั้น)(ดี|ถูก|แพง)(กว่า)?/u',
            '/(ถูก|แพง)(กว่า|ที่สุด)/u',
        ];
    ...
    /**
     * 🚀 Handle follow-up question with LLM context understanding
     */
    protected function handleFollowUpWithContext(string $text, array $config, array $context): ?array
    {
        $traceId = $context['trace_id'] ?? '';
        ...
        // Get conversation history (more history for follow-up context)
        $history = $this->chatService->getHistoryForLLM($context, 15);

        if (empty($history)) {
            // No history = can't understand follow-up context
            return ['reply' => 'ขออภัยค่ะ ช่วยบอกรายละเอียดเพิ่มเติมได้ไหมคะ? เช่น ชื่อสินค้าหรือรหัสสินค้าค่ะ'];
        }

        // Get last viewed product from quick state
        $platformUserId = $context['platform_user_id'] ?? '';
        $channelId = $context['channel']['id'] ?? 0;
        $lastProduct = $this->chatService->getQuickState('last_viewed_product', $platformUserId, $channelId);
        ...
        $shopName = $config['shop_name'] ?? 'ร้านค้า';

        $systemPrompt = <<<PROMPT
คุณคือผู้ช่วยร้าน "{$shopName}" ที่ฉลาดและเข้าใจบริบท

📌 งานของคุณ:
ลูกค้าถามคำถาม follow-up ที่อ้างอิงสิ่งที่คุยก่อนหน้า เช่น "อันนี้ผ่อนได้ไหม", "มีสีอื่นไหม"
คุณต้อง:
1. ดูประวัติสนทนาเพื่อเข้าใจว่า "อันนี้" หรือ "สินค้า" หมายถึงอะไร
2. ตัดสินใจว่าต้องเรียก function ใด
3. ถ้าเข้าใจ context → เรียก function ที่เหมาะสม
4. ถ้าไม่มี context เพียงพอ → ใช้ general_response ขอข้อมูลเพิ่ม

📌 ตัวอย่าง:
- ประวัติ: "พบ Rolex Submariner ราคา 450,000 บาท"
  ลูกค้า: "ผ่อนได้ไหม"
  → calculate_installment(price=450000)

- ประวัติ: "Rolex มีสีดำ สีน้ำเงิน"  
  ลูกค้า: "มีสีอื่นไหม"
  → search_products(keyword="Rolex") เพื่อหา variant อื่น

- ประวัติ: "Rolex Submariner"
  ลูกค้า: "อันนี้เหลือไหม"  
  → check_product_stock พร้อม context จาก Rolex
PROMPT;

        // Add last product context if available
        if ($lastProduct && !empty($lastProduct['value'])) {
            $p = $lastProduct['value'];
            $systemPrompt .= "\n\n🔖 สินค้าที่ลูกค้าเพิ่งดู:\n";
            $systemPrompt .= "- ชื่อ: " . ($p['name'] ?? $p['title'] ?? 'N/A') . "\n";
            $systemPrompt .= "- รหัส: " . ($p['code'] ?? $p['product_code'] ?? 'N/A') . "\n";
            $systemPrompt .= "- ราคา: " . ($p['price'] ?? 'N/A') . " บาท\n";
            $systemPrompt .= "- ID: " . ($p['id'] ?? 'N/A');
        }

        // Build prompt
        $prompt = $systemPrompt;
        $prompt .= "\n\n📜 ประวัติการสนทนา:\n{$history}";
        $prompt .= "\n\n👤 ลูกค้า (follow-up): {$text}";
        ...
        $payload = [
            ...
            'generationConfig' => [
                'temperature' => 0.2, // Lower for more deterministic context understanding
                'maxOutputTokens' => 1024,
            ],
            'toolConfig' => [
     ...
     * 
     * Patterns like "แล้วก็", "แล้วก็ถาม", "ด้วย", "พร้อมกับ"
     */
    protected function isMultiIntentMessage(string $text): bool
    {
        $text = mb_strtolower(trim($text), 'UTF-8');
        $len = mb_strlen($text, 'UTF-8');

        // Too short = probably not multi-intent
        if ($len < 15) {
            return false;
        }
        ...
        $multiIntentPatterns = [
            ...
            // "Also" patterns
            '/(อีก|เพิ่ม)(อย่าง|เรื่อง)/u',
            '/นอกจากนี้/u',

            // Common combos
            '/(ดู|หา|ค้นหา).*(แล้ว|แล้วก็).*(ถาม|เช็ค|ราคา|ผ่อน|ส่ง)/u',
            '/(สนใจ|อยากได้).*(แล้ว|แล้วก็).*(ถาม|ผ่อน|ราคา)/u',
        ];

        foreach ($multiIntentPatterns as $pattern) {
            if (preg_match($pattern, $text)) {
                return true;
            }
        }
    ...
    /**
     * 🚀 Handle message with multiple intents using LLM parallel function calling
     */
    protected function handleMultiIntent(string $text, array $config, array $context): ?array
    {
        $traceId = $context['trace_id'] ?? '';
        ...
        // Build multi-intent prompt
        $shopName = $config['shop_name'] ?? 'ร้านค้า';

        $systemPrompt = <<<PROMPT
คุณคือผู้ช่วยร้าน "{$shopName}" ที่ชาญฉลาด

📌 ลูกค้าส่งข้อความที่มีหลายคำขอในครั้งเดียว
คุณต้อง:
1. วิเคราะห์ว่ามีคำขอกี่อย่าง
2. เรียก function ที่เหมาะสมสำหรับแต่ละคำขอ (สามารถเรียกหลาย function ได้)
3. รวมผลลัพธ์เป็นคำตอบเดียว

📌 ตัวอย่าง:
- "ดูนาฬิกาสีดำ แล้วก็ถามราคาผ่อนด้วย"
  → เรียก 2 functions:
    1. search_products(keyword="นาฬิกาสีดำ")
    2. calculate_installment(price=<จากผลลัพธ์ search>)

- "มีกระเป๋า LV ไหม แล้วส่งได้เมื่อไหร่"
  → เรียก 2 functions:
    1. search_products(keyword="กระเป๋า LV")
    2. get_store_info(info_type="shipping")

- "เช็คยอดผ่อน แล้วก็ดูสินค้าใหม่ด้วย"
  → เรียก 2 functions:
    1. check_installment()
    2. search_products(keyword="สินค้าใหม่")

📌 สำคัญ:
- ถ้า function หนึ่งต้องใช้ผลลัพธ์จากอีก function → เรียกตามลำดับ
- ถ้าเป็นอิสระต่อกัน → เรียกพร้อมกันได้
PROMPT;
        ...
        // Build Gemini request with parallel function calling
        $payload = [
            'contents' => [
                [
                    'role' => 'user',
            ...
            'generationConfig' => [
                'temperature' => 0.3,
                'maxOutputTokens' => 2048, // More tokens for multiple results
            ],
            'toolConfig' => [
                'functionCallingConfig' => ['mode' => 'AUTO']
            ]
        ...
        return $this->combineMultiIntentResults($results, $config, $context);
    }

    /**
     * 🚀 Combine multiple function results into a cohesive response
     */
    protected function combineMultiIntentResults(array $results, array $config, array $context): array
    {
        $replies = [];
            ...
            switch ($type) {
                case 'product_list':
                    if (!empty($result['products'])) {
                        ...
                        // Store for later use (e.g., installment calculation)
                        $this->lastProductForMultiIntent = $product;

                        // Format product response
                        $platform = $context['platform'] ?? 'line';
                        $productCard = $this->formatProductSearchResponse($products, $platform, $config);
                        $hasProductCard = true;
                    } else {
                        $replies[] = $result['message'] ?? 'ไม่พบสินค้าค่ะ';
                    }
                    break;

                case 'installment_calculation':
                    // ✅ FIX: Don't show calculation directly - redirect to Product Card
                    // In multi-intent, the product card will already be shown
                    // Just add a hint that installment is available

                    $installmentConfig = $config['installment'] ?? [];
                    $periods = $installmentConfig['periods'] ?? 3;
                    $feePercent = $installmentConfig['fee_percent'] ?? 3;
                    $deliveryRule = $installmentConfig['delivery_rule'] ?? 'ผ่อนครบรับของ';

                    // Only show hint, not full calculation (Product Card is primary)
                    $msg = "💳 ผ่อนได้ {$periods} งวด +{$feePercent}% งวดแรก | {$deliveryRule}";
                    $replies[] = $msg;
                    break;
                ...
                case 'stock_status':
                    $name = $result['product_name'] ?? 'สินค้า';
                    $inStock = $result['in_stock'] ?? false;
                    $qty = $result['quantity'] ?? 0;
                    if ($inStock) {
                        $replies[] = "✅ {$name} มีอยู่ค่ะ เหลือ {$qty} ชิ้น";
                    } else {
                        $replies[] = "❌ {$name} หมดสต็อกค่ะ";
                    }
                    break;
        ...
        // If we have a product card, return it with additional info
        if ($hasProductCard && $productCard) {
            // Add text replies to the product card response
            if (!empty($replies)) {
                $additionalText = implode("\n\n", array_filter($replies));
                if (!empty($additionalText)) {
                    // Prepend to product card or add as separate message
                    if (is_array($productCard['reply'])) {
                        $productCard['additional_text'] = $additionalText;
                    } else {
                        $productCard['reply'] = $additionalText . "\n\n" . $productCard['reply'];
                    }
                }
            }
            return $productCard;
        ...
        // No product card - combine text replies
        $combinedReply = implode("\n\n", array_filter($replies));
        if (empty($combinedReply)) {
            $combinedReply = 'รับทราบค่ะ ดำเนินการให้แล้วนะคะ';
        }

        return ['reply' => $combinedReply];
    }

    // Property to store product for multi-intent
    protected $lastProductForMultiIntent = null;

    /**
     * Build system prompt for function calling
     */
    protected function buildFunctionCallingSystemPrompt(array $config): string
    {
        $shopName = $config['shop_name'] ?? $config['store']['name'] ?? 'ร้านค้า';
        $persona = $config['persona'] ?? [];
        $personality = $persona['identity'] ?? ($config['personality'] ?? 'เป็นมิตร สุภาพ ช่วยเหลือดี');

        // === Build context from ENTIRE config (dynamic, not hardcoded) ===
        $contextParts = [];

        // Store info
        $store = $config['store'] ?? [];
        if (!empty($store)) {
            $storeLines = ["📍 ข้อมูลร้าน:"];
            if (!empty($store['name']))
                $storeLines[] = "• ชื่อ: {$store['name']}";
            if (!empty($store['tagline']))
                $storeLines[] = "• ขาย: {$store['tagline']}";
            if (!empty($store['hours']))
                $storeLines[] = "• เวลาเปิด: {$store['hours']}";
            if (!empty($store['location'])) {
                $loc = is_array($store['location']) ? ($store['location']['short'] ?? json_encode($store['location'])) : $store['location'];
                $storeLines[] = "• ที่ตั้ง: {$loc}";
            }
            if (!empty($store['phone']))
                $storeLines[] = "• โทร: {$store['phone']}";
            if (!empty($store['product_categories'])) {
                $cats = implode(', ', $store['product_categories']);
                $storeLines[] = "• หมวดหมู่: {$cats}";
            }
            if (!empty($store['brands'])) {
                $brands = implode(', ', array_slice($store['brands'], 0, 5));
                $storeLines[] = "• แบรนด์: {$brands}";
            }
            $contextParts[] = implode("\n", $storeLines);
        }

        // Features/Policies from config
        $features = $config['features'] ?? [];
        if (!empty($features)) {
            $featureLines = ["📋 บริการ/นโยบาย:"];

            if (!empty($features['installment'])) {
                $inst = $features['installment'];
                $periods = $inst['periods'] ?? 3;
                $fee = $inst['fee_percent'] ?? 3;
                $delivery = $inst['delivery_on_completion'] ? 'ผ่อนครบรับของ' : 'รับของทันที';
                $featureLines[] = "• ผ่อน: {$periods} งวด, ค่าธรรมเนียม {$fee}% งวดแรก, {$delivery}";
            }
            if (!empty($features['deposit'])) {
                $dep = $features['deposit'];
                $pct = $dep['percent'] ?? 10;
                $days = $dep['hold_days'] ?? 14;
                $featureLines[] = "• มัดจำ: {$pct}% ถือสินค้า {$days} วัน";
            }
            if (!empty($features['pawn'])) {
                $pawn = $features['pawn'];
                $interest = $pawn['interest_monthly'] ?? 2;
                $featureLines[] = "• รับจำนำ: ดอกเบี้ย {$interest}%/เดือน";
            }

            $contextParts[] = implode("\n", $featureLines);
        }

        // Response templates (if any)
        $templates = $config['response_templates'] ?? [];
        if (!empty($templates)) {
            $tplLines = ["💬 ข้อความมาตรฐาน:"];
            foreach (array_slice($templates, 0, 5) as $key => $val) {
                if (is_string($val)) {
                    $short = mb_substr($val, 0, 50, 'UTF-8');
                    $tplLines[] = "• {$key}: \"{$short}...\"";
                }
            }
            $contextParts[] = implode("\n", $tplLines);
        ...
        return <<<PROMPT
คุณคือผู้ช่วยร้าน "{$shopName}" ที่ชาญฉลาดมาก
บุคลิก: {$personality}
{$contextSection}

📌 หน้าที่หลัก:
วิเคราะห์ข้อความลูกค้า + ประวัติการสนทนา แล้วเรียก function ที่เหมาะสม

� กฎสำคัญที่สุด - CONTEXT CONTINUITY:
ต้องอ่านประวัติการสนทนาและรวมข้อมูลจากข้อความก่อนหน้า!

ตัวอย่าง Context Continuity:
| ประวัติ | ข้อความปัจจุบัน | การวิเคราะห์ที่ถูกต้อง |
|---------|-----------------|----------------------|
| "หานาฬิกา Rolex" → แสดงสินค้า | "ขอราคาไม่เกิน 100k" | search_products: keyword="นาฬิกา", price_max=100000 (รู้ว่าหานาฬิกาอยู่) |
| "ขอราคาไม่เกิน 100k" | "นาฬิกา" | search_products: keyword="นาฬิกา", price_max=100000 (จำ budget ไว้) |
| "หานาฬิกา" → "ราคาไม่เกิน 100k" | "มีแบบอื่นอีกไหม" | search_products: keyword="นาฬิกา", price_max=100000 (จำทั้งคู่) |
| "ดูสินค้าราคาหลักหมื่น" | "นาฬิกา Rolex" | search_products: keyword="Rolex นาฬิกา", price_max=99999 (รวม budget) |

📌 การเรียก search_products:
- ต้องวิเคราะห์ keyword ให้ถูกต้อง (ไม่ใส่คำถามเข้าไป)
- ⚠️ ถ้าลูกค้าเคยระบุราคาในบทสนทนา → ใส่ price_max ด้วย!
- ⚠️ ถ้าลูกค้าเคยพูดถึงประเภทสินค้า → รวมเข้า keyword!
- ถ้าพิมพ์ผิด ให้แก้ให้ถูก เช่น "กำไร" → "กำไล"

📌 ตัวอย่างการวิเคราะห์:
| ข้อความ | function | parameters |
|---------|----------|------------|
| "มีนาฬิกาสีดำไหม" | search_products | keyword="นาฬิกา สีดำ" |
| "นาฬิกา ราคาไม่เกิน 500000" | search_products | keyword="นาฬิกา", price_max=500000 |
| "นาฬิกา งบ 5 แสน" | search_products | keyword="นาฬิกา", price_max=500000 |
| "มีแหวนทองแดงไหม" | search_products | keyword="แหวน ทองแดง" |
| "กำไรข้อมือ" | search_products | keyword="กำไล ข้อมือ" (แก้ กำไร→กำไล) |
| "อยากได้พระเครื่อง" | search_products | keyword="พระเครื่อง" |
| "มีพระนางกวักด้วยหรอ" | search_products | keyword="พระนางกวัก" |
| "มีสินค้าอะไรบ้าง" | search_products | keyword="สินค้า" (หาทั้งหมด) |
| "ขอดูสินค้า" | search_products | keyword="สินค้า" |
| "Rolex" (ต่อจากบทสนทนาเรื่องนาฬิกา) | search_products | keyword="Rolex นาฬิกา" |
| "อันนั้นเหลือไหม" | check_product_stock | ดูจาก context |
| "สนใจอันนี้" | create_order | ดูสินค้าจาก context |
| "ผ่อนได้ไหม" | calculate_installment | ดูราคาจาก context |
| "สถานะออเดอร์" | get_order_status | - |
| "ขอคุยกับแอดมิน" | request_admin_handoff | - |
| "ร้านเปิดกี่โมง" | get_store_info | - |
| "ร้านอยู่ที่ไหน" | get_store_info | - |
| "ติดต่อร้านได้ไง" | get_store_info | - |
| "สวัสดีค่ะ" | general_response | response_text="สวัสดีค่ะ ยินดีให้บริการค่ะ 😊" |
| "ขอบคุณนะ" | general_response | response_text="ยินดีค่ะ มีอะไรให้ช่วยเพิ่มไหมคะ? 💛" |

📌 กฎสำคัญ:
1. ทุกคำถามเกี่ยวกับสินค้า → search_products (ไม่ใช่ general_response)
2. "มี...ไหม", "อยากได้...", "หา...", "ดู..." = ค้นหาสินค้า
3. ดู context: ถ้าเพิ่งคุยเรื่อง Rolex แล้วลูกค้าพูดว่า "สีดำ" → keyword="Rolex สีดำ"
4. ⚠️ จำ budget: ถ้าลูกค้าเคยบอก "งบไม่เกิน 100k" → ทุก search ต่อจากนั้นต้องใส่ price_max=100000
5. ราคา: "งบ 5 แสน" = 500000, "ไม่เกิน 100k" = 100000, "หลักหมื่น" = price_max=99999
6. แก้พิมพ์ผิด: "กำไร" → "กำไล", "โลเล็ก" → "Rolex"
PROMPT;
    }

    /**
     * Process function execution result and return appropriate response
     */
    protected function processFunctionResult(array $result, string $functionName, array $config, array $context): ?array
    {
        $type = $result['type'] ?? 'unknown';
        ...
        switch ($type) {
            case 'product_list':
                ...
                if (!empty($result['products'])) {
                    $products = $result['products'];
                    $product = $products[0];
                    $totalFound = $result['total_found'] ?? count($products);
                    ...
                    $platform = $context['platform'] ?? 'line';
                    $response = $this->formatProductSearchResponse($products, $platform, $config);

                    // Add hint if there are more products
                    if ($totalFound > count($products)) {
                        $moreCount = $totalFound - count($products);
                        $hint = "\n\n💡 มีอีก {$moreCount} รายการ บอกรายละเอียดเพิ่มได้เลยค่ะ (แบรนด์/ราคา/สี)";
                        if (is_string($response['reply'] ?? null)) {
                            $response['reply'] .= $hint;
                        }
                    }
            ...
            case 'too_many_results':
                // 🚀 Hybrid Response: แสดง sample + ถามเพิ่ม
                $keyword = $result['keyword'] ?? 'สินค้า';
                $total = $result['total_found'] ?? 0;
                $topBrands = $result['top_brands'] ?? [];
                $priceRanges = $result['price_ranges'] ?? [];
                $samples = $result['sample_products'] ?? [];

                $msg = "📦 พบ \"{$keyword}\" มากกว่า {$total} รายการเลยค่ะ!\n\n";

                // Show top brands if available
                if (!empty($topBrands)) {
                    $msg .= "🏷️ แบรนด์ยอดนิยม:\n";
                    foreach (array_slice($topBrands, 0, 4) as $brand) {
                        $msg .= "• {$brand}\n";
                    }
                    $msg .= "\n";
                }

                // Show price range hints
                $msg .= "💰 ช่วงราคา:\n";
                if (($priceRanges['under_100k'] ?? 0) > 0) {
                    $msg .= "• ต่ำกว่า 100,000 บาท\n";
                }
                if (($priceRanges['100k_500k'] ?? 0) > 0) {
                    $msg .= "• 100,000 - 500,000 บาท\n";
                }
                if (($priceRanges['over_500k'] ?? 0) > 0) {
                    $msg .= "• มากกว่า 500,000 บาท\n";
                }

                $msg .= "\n💡 ช่วยบอกเพิ่มได้ไหมคะ?\n";
                $msg .= "• พิมพ์ชื่อแบรนด์ เช่น \"Rolex\"\n";
                $msg .= "• บอกงบ เช่น \"งบไม่เกิน 5 แสน\"\n";
                $msg .= "• บอกสี/สไตล์ เช่น \"สีดำ\" \"ทรง Sport\"";
            ...
            case 'stock_status':
                ...
                if ($inStock) {
                    $priceText = $price > 0 ? " ราคา " . number_format($price) . " บาท" : "";
                    return ['reply' => "✅ {$name} ยังมีอยู่ค่ะ เหลือ {$qty} ชิ้น{$priceText} สนใจสั่งเลยไหมคะ?"];
                } else {
                    return ['reply' => "❌ ขออภัยค่ะ {$name} หมดสต็อกแล้วค่ะ ต้องการให้แจ้งเมื่อมีของเข้าไหมคะ?"];
                }
            ...
            case 'checkout_started':
                // Trigger checkout confirmation flow
                return $this->handleCheckoutConfirm($config, $context);
            ...
            case 'admin_handoff':
            case 'store_info':
                return ['reply' => $result['message'] ?? 'รับทราบค่ะ'];

            case 'installment_calculation':
                // ✅ FIX: Don't calculate directly - redirect to Product Card flow
                // V5 should help understand context, but still use Product Card → Checkout Flow

                $platformUserId = $context['platform_user_id'] ?? '';
                $channelId = $context['channel']['id'] ?? 0;
                $lastProduct = $this->chatService->getQuickState('last_viewed_product', $platformUserId, $channelId);

                if (!empty($lastProduct['value'])) {
                    ...
                    // ✅ Return Product Card (V4 flow)
                    $platform = $context['platform'] ?? 'line';

                    // Get installment config for additional info
                    $installmentConfig = $config['installment'] ?? [];
                    $periods = $installmentConfig['periods'] ?? 3;
                    $feePercent = $installmentConfig['fee_percent'] ?? 3;
                    $deliveryRule = $installmentConfig['delivery_rule'] ?? 'ผ่อนครบรับของ';

                    // Add installment hint to product
                    $product['installment_hint'] = "ผ่อนได้ {$periods} งวด +{$feePercent}% งวดแรก | {$deliveryRule}";
                ...
                $msg = "💳 ร้านเรามีบริการผ่อนชำระค่ะ\n\n";
                $msg .= "📌 เงื่อนไขผ่อน:\n";
                $msg .= "• ผ่อน {$periods} งวด\n";
                $msg .= "• ค่าธรรมเนียม {$feePercent}% งวดแรก\n";
                $msg .= "• {$deliveryRule}\n\n";
                $msg .= "💡 สนใจสินค้าตัวไหนคะ? ส่งรหัสหรือชื่อสินค้ามาได้เลยค่ะ 🛍️";
    ...
    protected function isLlmEnabled(array $config): bool
    {
        ...
        if (
            !empty($config['backend_api']['enabled']) &&
    ...
    protected function detectEarlyCheckout(string $text, array $context): ?array
    {
        ...
        $text = mb_strtolower($originalText, 'UTF-8');

        // === Pattern 1: Interest with product code (from quick reply) ===
        // "สนใจ ROL-DAY-001" → extract code and start checkout
        if (preg_match('/^สนใจ\s+([A-Z0-9\-]{3,20})$/iu', $originalText, $codeMatch)) {
            ...
            $productResult = $this->productService->getByCode($productCode, $config, $context);

            if ($productResult['ok'] && !empty($productResult['product'])) {
            ...
            // Product not found - return null to let LLM handle
            return null;
        }

        // === Pattern 2: Generic interest (use recently viewed) ===
        $interestPatterns = [
