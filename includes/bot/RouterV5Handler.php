<?php
/**
 * RouterV5Handler - Refactored message router using service-based architecture
 * 
...
class RouterV5Handler implements BotHandlerInterface
{
    protected $db;
...
PROMPT;
        ...
        foreach ($results as $item) {
            ...
            switch ($type) {
                ...
                case 'store_info':
                    $info = $result['info'] ?? [];
                    $parts = [];
                    if (!empty($info['location'])) $parts[] = "📍 " . $info['location'];
                    if (!empty($info['hours'])) $parts[] = "🕐 " . $info['hours'];
                    if (!empty($info['phone'])) $parts[] = "📞 " . $info['phone'];
                    if (!empty($parts)) {
                        $replies[] = implode("\n", $parts);
                    }
                    break;
    ...
    protected function buildFunctionCallingSystemPrompt(array $config): string
    {
        ...
        if (!empty($store)) {
            ...
            if (!empty($store['name'])) $storeLines[] = "• ชื่อ: {$store['name']}";
            if (!empty($store['tagline'])) $storeLines[] = "• ขาย: {$store['tagline']}";
            if (!empty($store['hours'])) $storeLines[] = "• เวลาเปิด: {$store['hours']}";
            ...
            if (!empty($store['phone'])) $storeLines[] = "• โทร: {$store['phone']}";
