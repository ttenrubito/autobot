 ...
 * Tests critical logic without full mocking (uses Logger/Database mocks from bootstrap)
 * 
 * Run: /opt/lampp/bin/php tests/bot/RouterV1SimpleTest.php
 */

// Load bootstrap with mock classes (MUST be first)
...
// Note: Admin command ONLY works when is_admin=true (message from admin, not customer)
// AND requires a valid session_id (from findOrCreateSession)
echo "Test 3: Admin command triggers handoff\n";
echo "---\n";
try {
    $handler = new RouterV1Handler();
    
    // Mock session data - admin command requires session_id to work
    $mockDb = Database::getInstance();
    $mockDb->setMockData('queryOne', [
        'id' => 123,
        'channel_id' => 999,
        'external_user_id' => 'test_user',
    ...
    $context = [
        ...
        'channel' => ['id' => 999, 'platform' => 'facebook'],
        'external_user_id' => 'test_user',
        'user' => ['external_user_id' => 'test_user', 'is_admin' => true],
        'is_admin' => true, // Admin command requires admin sender
