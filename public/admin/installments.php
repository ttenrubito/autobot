            ...
            scheduleHtml += `
                <div class="schedule-item ${statusClass}">
                    ...
                    <div class="schedule-info">
                        <div class="schedule-amount">${formatMoney(contract.amount_per_period)}</div>
...
async function sendReminder() {
    ...
    try {
        // Use push notification API
        const response = await adminApiCall(PATH.api('api/webhook/push-notify/send'), {
            method: 'POST',
            body: JSON.stringify({
                platform: currentInstallmentData.platform,
                platform_user_id: currentInstallmentData.platform_user_id,
                template_type: 'installment_reminder',
                variables: {
                    customer_name: currentInstallmentData.customer_name,
                    period_number: (currentInstallmentData.paid_periods || 0) + 1,
                    total_periods: currentInstallmentData.total_periods,
                    amount: currentInstallmentData.amount_per_period,
                    due_date: formatDate(currentInstallmentData.next_due_date),
                    product_name: currentInstallmentData.product_name
                }
            })
        });
        ...
        if (response.success) {
            ...
            showToast(response.message || 'ส่งแจ้งเตือนไม่สำเร็จ', 'error');
        }
    } catch (error) {
