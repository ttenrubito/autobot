                ...
                <div class="detail-section">
                    ...
                    <div class="detail-grid">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="itemName" name="item_name" class="form-input" required placeholder="เช่น สร้อยคอทองคำ 2 บาท">
                        </div>
                    </div>
                    <div class="form-group">
                        <label for="itemDescription">สภาพ/หมายเหตุสินค้า</label>
                        <textarea id="itemDescription" name="item_description" class="form-input" rows="2" placeholder="สภาพสินค้าและหมายเหตุ"></textarea>
                    </div>
                </div>
                ...
                <div class="detail-section">
                    ...
                    <div class="detail-grid">
                        <div class="form-group">
                            ...
                            <input type="number" id="loanAmount" name="loan_amount" class="form-input" required placeholder="10000" min="0" step="1">
                        </div>
                        <div class="form-group">
                            <label for="interestRate">อัตราดอกเบี้ย (% ต่อเดือน) <span class="required">*</span></label>
                            <input type="number" id="interestRate" name="interest_rate" class="form-input" required placeholder="2" min="0" step="0.1" value="2">
                        </div>
                    </div>
                    <div class="detail-grid">
                        <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="pawnNotes" name="notes" class="form-input" rows="2" placeholder="หมายเหตุเพิ่มเติม"></textarea>
                    </div>
                </div>
            </form>
        </div>
...
.status-redeemed { background: #d1fae5; color: #059669; }
.status-forfeited { background: #f3f4f6; color: #6b7280; }
...
.pawn-info-row:last-child { border-bottom: none; }
.pawn-info-label { color: var(--color-gray); }
.pawn-info-value { font-weight: 600; }
...
.payment-history h4 { margin-bottom: 1rem; color: var(--color-dark); }
.payment-item {
    display: flex;
    justify-content: space-between;
    align-items: center;
...
.payment-item-date { color: var(--color-gray); font-size: 0.85rem; }
...
async function submitCreatePawn() {
    ...
    if (!itemType || !itemName || !loanAmount || !interestRate) {
        showError('กรุณากรอกข้อมูลให้ครบ');
        return;
    }
    
    try {
        const response = await fetch('/api/admin/pawns.php', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': 'Bearer ' + getToken()
